...
export class PokemonesGridComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.typeSubscription = this.pokemonService.onTypeSelected().subscribe(typeId => {
			this.type = typeId;
			this.offset = 1;
			this.fetchPokemonsBasedOnFilters();
		});

		this.generationSubscription = this.pokemonService.onGenerationSelected().subscribe(generation => {
			this.generation = generation;
			this.offset = 1;
			this.fetchPokemonsBasedOnFilters();
		});

		this.typeSubscription = this.pokemonService.onWeaknessSelected().subscribe(weakness => {
	...
	fetchPokemonsByType(): void {
		this.loading = true;
		this.pokemonService.getPokemonByType(this.type, this.weakness).subscribe(pokemonNames => {
			this.pokemonNames = pokemonNames;
			if (this.pokemonNames.length === 0) {  // aqui se deberia presentar un mensaje indicando que no se encontraron pokemones
				this.loading = false;
				return;
			}
			this.pokemons = [];
			for (let i = this.offset - 1; i <= this.offset + this.limit - 1; i++) {
				...
				this.pokemonService.getSinglePokemon(this.pokemonNames[i], this.generation).subscribe(pokemon => {
					this.pokemons.push(pokemon);
					this.pokemons = this.pokemons.sort((first, second) => first.id - second.id)
					if (this.pokemons.length === this.limit + 1) {
						this.loading = false;
