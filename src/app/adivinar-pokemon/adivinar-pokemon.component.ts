...
export class AdivinarPokemonComponent {
  ...
  comenzarJuego(iniciarTimer: boolean = false): void {
    ...
    this.pokemonService
      .getPokemon(nombrePokemon.toLowerCase().trim())
      .subscribe(
        (data) => {
          if (iniciarTimer) {
            this.comenzarTimer();
  ...
  obtenerNombrePokemonAleatorio(pokemonCorrecto: string | null = null): string {
    let pokemonesDisponibles: string[] = nombresPokemon.filter(
      (pokemon) => !this.listaPokemons.includes(pokemon)
    );
    if (pokemonCorrecto) {
      pokemonesDisponibles = pokemonesDisponibles.filter(
        (pokemon) => pokemon !== pokemonCorrecto
      );
    }
    const pokemonAleatorio: string =
      pokemonesDisponibles[
        Math.floor(Math.random() * pokemonesDisponibles.length)
      ];
