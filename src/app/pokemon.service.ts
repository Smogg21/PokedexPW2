...
export class PokemonService {
  ...
  getRandomPokemon(): Observable<any> {
    ...
    return this.http.get(`${this.apiUrl}/pokemon/${randomId}`);
  }
  getSinglePokemon(pokemonName: string, generation: string): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      map((data) => {
        ...
        let mainImg = version
          ? version[Object.keys(version)[0]].front_default
  ...
  getPokemonTypes(
    language: string = 'es'
  ): Observable<{ name: string | undefined; id: number }[]> {
    return this.http
      .get<{ results: { url: string }[] }>(`${this.apiUrl}/type/`)
      .pipe(
        switchMap((response) => {
        ...
        map((types) => {
          return types
            ...
            .map((type) => {
              const translatedName = type.names.find(
  ...
  getPokemonByType(typeId: number, weakness: number): Observable<string[]> {
    if (weakness === -1) {
      return this.http
        .get<any>(`${this.apiUrl}/type/${typeId}`)
        .pipe(
    ...
    return forkJoin([
      ...
      switchMap(([typeResponse, weaknessResponse]) => {
        const pokemonsWithType: string[] = typeResponse.pokemon.map(
          (p: any) => p.pokemon.name
        );
        const weaknessTypes: string[] =
          weaknessResponse.damage_relations.double_damage_from.map(
            (type: any) => type.name
          );

        const weaknessRequests: Observable<any>[] = weaknessTypes.map(
          (typeName: string) =>
        ...
        return forkJoin(weaknessRequests).pipe(
          map((weaknessResponses: any[]) => {
            const pokemonsNotAllowed: string[] = [].concat(
              ...weaknessResponses.map((response: any) =>
                response.pokemon.map((p: any) => p.pokemon.name)
              )
            );
            let filteredPokemons = pokemonsWithType;
            if (pokemonsNotAllowed.length > 0) {
              filteredPokemons = pokemonsWithType.filter(
                (pokemon: any) => !pokemonsNotAllowed.includes(pokemon)
              );
            }
  ...
  getPokemonGenerations(): Observable<
    { value: string | undefined; id: string }[]
  > {
    return this.http.get<any>(`${this.apiUrl}/generation/`).pipe(
      map((response) =>
        response.results.map((generation: any) => {
          const id = generation.url
            .split('/')
